...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        assert width > 0 and height > 0 and dirt_amount >= 0, 'Invalid parameters'
        self.width = width
        self.height = height
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.get_tiles()[(x, y)] > 0:
            if capacity < self.get_tiles()[(x, y)]:
                self.get_tiles()[(x, y)] -= capacity
            else:
                self.get_tiles()[(x, y)] = 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        for x, y in xyrange(self.get_width(), self.get_height()):
    ...
    def is_position_in_room(self, pos):
        ...
        """
        return 0 <= math.floor(pos.get_x()) < self.get_width() \
                and 0 <= math.floor(pos.get_y()) < self.get_height()
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    return total_num_steps / num_trials
