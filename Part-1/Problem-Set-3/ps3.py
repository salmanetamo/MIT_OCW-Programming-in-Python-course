...
def get_word_score(word, n):
    ...
    secondComponent = 0
    for char in word:
        firstComponent += SCRABBLE_LETTER_VALUES.get(char,0)
...
def display_hand(hand):
    ...
    for letter in hand.keys():
        for j in range(hand[letter]):
            list_to_string.append(letter)
    
    list_to_string = ' '.join(list_to_string)
...
def is_valid_word(word, hand, word_list):
    ...
    if word not in word_list:
        if word.find('*') == -1:
            ...
            wildcards_index = word.find('*')
            vowels_checked = 0            
            for vowel in VOWELS:                       
                word = list(word)
                word[wildcards_index] = vowel
                word = ''.join(word)
                if word in word_list:
                    word = list(word)
                    word[wildcards_index] = '*'
                    break
                vowels_checked += 1
            if vowels_checked >= 5:
                return False 
    ...
    for char in word:
        if char not in copy_of_hand.keys():
            return False
        copy_of_hand[char] -= 1
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while number_of_hands > 0:
        ...
        if number_of_substitutes == 1:
            substitution = input('Would you like to subsitute a letter?').lower()
            if substitution == 'yes':
                number_of_substitutes -= 1
